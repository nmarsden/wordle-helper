...
const suggestionUseButtonElem = document.getElementById('useButton');
...
const onGuessLetterClicked = (index) => {
    return (event) => {
        ...
        const oldHint = (currentHintIndex !== -1) ? AVAILABLE_HINTS[currentHintIndex] : '';
...
const initialHint = (letter, position) => {
    ...
    const absentLetters = guessLetters.filter(guessLetter => guessLetter.hint === 'absent').map(guessLetter => guessLetter.letter);
...
const addLetter = (letter) => {
    const position = currentGuessLetterIndex % 5;
    const hint = initialHint(letter, position);

    guessLetterElems[currentGuessLetterIndex].textContent = letter;
...
const suggestionHint = (letter, position) => {
    ...
    const presentLetters = guessLetters.filter(guessLetter => guessLetter.hint === 'present').map(guessLetter => guessLetter.letter);
...
const updateSuggestion = () => {
    ...
    const presentAndCorrectLetters = guessLetters.filter(guessLetter => ['present', 'correct'].includes(guessLetter.hint)).map(guessLetter => guessLetter.letter);
    ...
    const correctLetterAndPositions = guessLetters.filter(guessLetter => guessLetter.hint === 'correct').map(guessLetter => ({ letter: guessLetter.letter, position: guessLetter.position }));
    ...
    const presentLetterAndPositions = guessLetters.filter(guessLetter => guessLetter.hint === 'present').map(guessLetter => ({ letter: guessLetter.letter, position: guessLetter.position }));
    ...
    // show suggestion
    const suggestionLetters = (suggestion === '') ? new Array(5).fill('') : suggestion.split('');
    suggestionLetterElems.forEach((elem, index) => {
        elem.textContent = suggestionLetters[index];
        AVAILABLE_HINTS.forEach(hint => elem.classList.remove(`hint-${hint}`));
        const hint = suggestionHint(suggestionLetters[index], index);
        if (hint !== '') {
    ...
    if (isSuggestionUseDisabled()) {
        suggestionUseButtonElem.classList.add('disabled');
    } else {
